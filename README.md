...
This repository is an initial scaffold. It contains an installable Python
package, help/version entry points, development tooling, CI, and a provisional
design and roadmap. The ledger, worker integration, gate enforcement, Lean
verification, and replay engine are **planned, not implemented**.
...
Use `uv add` for new dependencies and commit the resulting lockfile. The scaffold
has no domain test suite yet. Add focused tests with the first behavior-bearing
slice; each acceptance path needs a negative case. CI currently checks lint,
...
| [src/warranted](src/warranted) | Python package and initial CLI |
| [.github/workflows/ci.yml](.github/workflows/ci.yml) | Reproducible scaffold checks |

The next implementation slice is a local evidence ledger that survives reopening,
deduplicates operation receipts, and exposes inspectable history. See
